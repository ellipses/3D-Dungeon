...
inline vec3::vec3 (const vec2& vv, float z) {
	v[0] = vv.v[0];
	v[1] = vv.v[1];
	v[2] = z;
}
...
inline vec4::vec4 (const vec2& vv, float z, float w) {
	v[0] = vv.v[0];
	v[1] = vv.v[1];
	v[2] = z;
	v[3] = w;
...
inline vec4::vec4 (const vec3& vv, float w) {
	v[0] = vv.v[0];
	v[1] = vv.v[1];
	v[2] = vv.v[2];
	v[3] = w;
...
inline mat3::mat3 (float a, float b, float c,
...
// note: entered in rows, but stored in columns
inline mat4::mat4 (float a, float b, float c, float d,
...
inline mat4 perspective (float fovy, float aspect, float nearv, float farv) {
	float fov_rad = fovy * ONE_DEG_IN_RAD;
	float range = tan (fov_rad / 2.0f) * nearv;
	float sx = (2.0f * nearv) / (range * aspect + range * aspect);
	float sy = nearv / range;
	float sz = -(farv + nearv) / (farv - nearv);
	float pz = -(2.0f * farv * nearv) / (farv - nearv);
...
//returns a ortho matrix in opengl projection style.Dont use near and far words because visual studio uses them or something.
//mat4 Ortho(left, right, bottom, top, near, far)
inline mat4 ortho(float left, float right, float bottom, float top, float nearv, float farv) {
	
	mat4 mat=identity_mat4();

	mat.m[0] = 2/(right-left);
	mat.m[5] = 2/(top-bottom);
	mat.m[10]= -2/(farv-nearv);
	mat.m[12]= -(left+right/(right-left));
	mat.m[13]= -((top+bottom)/(top-bottom));
	mat.m[14]= -((farv+nearv)/(farv-nearv));
...
inline versor::versor () { }

inline versor versor::operator/ (float rhs) {
	versor result;
	result.q[0] = q[0] / rhs;
	result.q[1] = q[1] / rhs;
	result.q[2] = q[2] / rhs;
...
inline versor versor::operator* (const versor& rhs) {
	versor result;
